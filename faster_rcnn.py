...
dataLabel = np.empty((len(valid_anchor_index), ), dtype=np.int32)
dataLabel.fill(-1)
print(dataLabel.size)  # (8940,)
dataLabel[max_ious < neg_iou_threshold] = 0  # anchor框内最大IOU值小于neg_iou_threshold，设为0
dataLabel[gt_argmax_ious] = 1  # anchor框有全局最大IOU值，设为1
dataLabel[max_ious >= pos_iou_threshold] = 1  # anchor框内最大IOU值大于等于pos_iou_threshold，设为1
...
# 随机获取n_pos个正例，
pos_index = np.where(dataLabel == 1)[0]
if len(pos_index) > n_pos:
   disable_index = np.random.choice(pos_index, size=(len(pos_index) - n_pos), replace=False)
   dataLabel[disable_index] = -1

n_neg = n_sample - np.sum(dataLabel == 1)
neg_index = np.where(dataLabel == 0)[0]

if len(neg_index) > n_neg:
   disable_index = np.random.choice(neg_index, size=(len(neg_index) - n_neg), replace = False)
   dataLabel[disable_index] = -1
print(np.sum(dataLabel == 1))  # 18个正例
print(np.sum(dataLabel == 0))# 256-18=238个负例
...
anchor_labels = np.empty((len(anchors),), dtype=dataLabel.dtype)
anchor_labels.fill(-1)
anchor_labels[valid_anchor_index] = dataLabel
